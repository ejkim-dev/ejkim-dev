...
public class PreferenceManager {
    ...
    private static final String PREFERENCES_NAME = "MYALBUMDATA";//저장소 이름
...
/*
    //생성자, SharedPreferences 저장소 이름
    public PreferenceManager(String PREFERENCES_NAME) {
    ...
    public static void saveArr(Context context, String key, ArrayList arrayList){
            ...
            /*   Gson gson = new Gson();
        String json = gson.toJson(MyAlbumAdapter.items);//items에 있는 자바 언어를 gson을 이용해 json으로 변환시켜줌
        preferenceManager.setString(mContext, LoginActivity.userID,json);*/
...
//        String text = PreferenceManager.getString(mContext, "test");
        String json = PreferenceManager.getString(mContext, LoginActivity.userID);//test : json에서 가져올 key와 기본값 설정
        Type type = new TypeToken<ArrayList<MyAlbum>>(){}.getType();//test
        items = gson.fromJson(json, type);//test
        //Nullpoint 예외처리 : 해당 key에 아무것도 없다면 리셋
        if(items == null){ items = new ArrayList<>();}//test*/
   ...
   /* public static String[] loadKeys(Context context, String[] keys){
        ...
        //key의 갯수
        Collection<?> col = prefs.getAll().keySet();
        Iterator<?> it = col.iterator();//모든 컬렉션 안에는 iterator() 메서드가 있다; 이터럴 객체 it 은 모든 객체를 가져온다

        while (it.hasNext()){
            keys[keyNum] = (String) it.next();
            keyNum++;
        }

        for (int i = 0 ; i < KeysNum(context) ; i ++){
    ...
    public static int KeysNum(Context context){
        int value = 0;
