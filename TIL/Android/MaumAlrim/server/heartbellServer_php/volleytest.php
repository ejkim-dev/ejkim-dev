...
//업로드한 유저 아이디 개수 카운트해서 채팅방 이름으로 저장
      ...
      //   echo "결과2 = ".$result->fetchColumn()+1;
        $room_id = $user_id.$end_time;


        try{
            // 4. SQL문을 실행하여 데이터를 MySQL 서버의 member 테이블에 저장합니다.
            $stmt = "INSERT INTO chatRoom(user_id, user_name, room_id, other_id, other_name, chat_category, summary, end_time)";
            $stmt .= "VALUES('{$user_id}', '{$user_name}', '{$room_id}', '{$other_id}', '{$other_name}', '{$chat_category}', '{$summary}', '{$end_time}')";
