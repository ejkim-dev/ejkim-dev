...
// $_SERVER['HTTP_USER_AGENT'] : php 접속 환경 정보에 strpos(문자열 포함 확인 함수)로 Androi가 있는지 확인
    ...
    if($android)//($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['submit']) || $android
    {
      /*			create table member(
			      ...
			      gender enum('M', 'F', 'X') default 'X' NOT NULL,
      ...
      // sogha586 는 임의적으로 비밀번호에 값을 더하여 실제 입력한 값과 다르게 암호화함
        ...
        if(!isset($errMSG)) // 모두 입력이 되었다면
        {
            try{
                ...
                $stmt = $con->prepare('INSERT INTO member(user_nickname, user_email, user_pw, birth_year, job, gender, reg_time) VALUES(:user_nickname, :user_email, :user_pw, :birth_year, :job, :gender, :reg_time)');
